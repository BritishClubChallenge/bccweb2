...
export const SCOPED_CASES: readonly CallSiteCase[] = [
  ...
  // Round writes routed through lib/roundTransitions.ts. These are coord-SCOPE
  ...
  { file: "roundTransitions.ts", handler: "createRound", endpoint: "createRound", tier: "standard", forbiddenKind: "coord-scope", setup: async () => ({ forbidden: await crossClubCoord(), request: { method: "POST", body: { date: "2026-06-01", siteId: randomUUID(), seasonYear: 2026, organisingClubId: randomUUID() } } }) },
  { file: "roundTransitions.ts", handler: "updateRound", endpoint: "updateRound", tier: "standard", forbiddenKind: "coord-scope", setup: async () => ({ forbidden: await crossClubCoord(), request: { method: "PUT", params: { id: (await roundForOtherClub("Proposed")).id }, body: { maxTeams: 4 } } }) },
